...
PDF_PATH = DATA_DIR / "2025 알기 쉬운 의료급여제도.pdf"
CACHE_PATH = DATA_DIR / "document_parse_result_2025.json"
FALLBACK_CACHE_PATH = DATA_DIR / "2025 알기 쉬운 의료급여제도.cache.json"
...
DENTURE_IMPLANT_TABLE_2025 = """| 구분 | 틀니 | 임플란트 |
| --- | --- | --- |
| 1종 | 5% | 10% |
| 2종 | 15% | 20% |"""
...
def normalize_known_broken_table_2025(md: str, inferred_heading: str, page_num: int) -> tuple[str, bool]:
    ...
    if page_num == 10 and "틀니" in body:
        return DENTURE_IMPLANT_TABLE_2025, False
...
if CACHE_PATH.exists():
    ...
    assert API_KEY, "UPSTAGE_API_KEY 환경변수가 설정되지 않았습니다."
...
def parse_section_2(pdf_path: Path) -> list[dict]:
    ...
    def flush():
        ...
        if current_q_num is not None and current_a_text:
            tables = []
            if current_page is not None and current_page not in used_table_pages:
...
output_path = BASE_DIR.parent / "data" / "chunks" / "all_chunks(2025).json"
