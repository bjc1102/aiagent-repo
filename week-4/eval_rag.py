...
RESULT_PATH = OUTPUT_DIR / "baseline_eval_results.json"
JSONL_RESULT_PATH = OUTPUT_DIR / "baseline_eval_results.jsonl"
        ...
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                ...
                if not isinstance(obj, dict):
                    raise ValueError(f"line {line_no}: JSON object가 아님")
                rows.append(obj)
            except Exception as e:
                print(f"[ERROR] JSON 파싱 실패: line {line_no}")
                print(line)
                raise e
    return rows
...
def judge_year_correct(
    question: str,
    dataset_source_year: str,
    retrieved_docs: List[Any],
) -> bool:
    question_year = extract_year_from_question(question) or str(dataset_source_year)

    retrieved_years = [
        str(doc.metadata.get("source_year", "unknown"))
    ...
    return question_year in retrieved_years
